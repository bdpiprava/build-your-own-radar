...
export class Renderer {
    ...
    constructor(config: Config, radar: RadarJSON) {
        this.c = config;
        this.radar = radar;
    ...
    private prepareBlipsForRendering(): Array<Blip> {
        const blips = new Array<Blip>()

        this.radar.quadrants.forEach((q: QuadrantJSON) => {
        ...
        const sorted = sort(blips, Renderer.blipComparator);
    ...
    private plotBlipsDetail(blips: Array<Blip>) {
        ...
        blips.forEach((blip: Blip) => {
            ...
            if (blip.quadrant != currentQuad) {
                ...
                currentPage = Renderer.createPage(this.c.WIDTH);
